...
if (flags.lsmod) {
	...
	const query = 'select path, major, minor, patch, extver, integrity, featpro, featreq, modreq, moddefer, modcaps from modules' + (where.length ? (' where ' + where.join(' and ')) : '');
	for (const [path, major, minor, patch, extver, integrity, featpro, featreq, modreq, moddefer, modcaps] of db.query(query, params)) {
