...
* (getter key else=elseBlock)  
  ...
  * RIC values: elseBlock  
  * Equivalent to { *receiver*(at *key* else=*elseBlock*) \!}, but marginally more efficient.  
...
* (indexEntries compact)  
  ...
  * If compact is true, keys are numeric instead of strings. The default is @f.  
* (indexKeys)  
...
* (setter key)  
  ...
  * Equivalent to { %0(set %1 to=\!0) }(fn *receiver key*)  
...
* create() \- A function to create a new reactive and return a new RIO for it (the JS equivalent of SysCL @c(get @reactive)(rio); used to propagate reactivity to auto-generated nested lists, for example).
