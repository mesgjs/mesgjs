...
* Blurring the line a bit between code blocks and comments:  
  * JavaScript embeds begin with "@js{" and end with "@}". Everything in between must be valid JavaScript. If JavaScript embedding is enabled at transpilation time, the JavaScript content will be included in the generated code. Otherwise, the JavaScript embed is treated like a comment.  
...
There are no retrieval shortcuts for @mps, so you will need to message the object directly for both retrieving and setting values.

## Global Shared Storage

Global values may be stored in a list object accessible by messaging object "@gss". There are no retrieval shortcuts for variables in this scope, so you will need to message the object directly for both retrieving and setting values.
