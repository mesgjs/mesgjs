...
| Quoted "strings" | 'hello, world' "hello, world" | Text within matching plain single or double quotes. |
...
| Namespace-at | %x \#x \!x // error if not set %?x \#?x \!?x // @u if not set | %(at x), \#(at x), \!(at x) shortcuts %(at x else=@u), etc. shortcuts |
| Code blocks | { block } // "non-returning" { block \!} // "returning" | When (run), evaluates the block. "Non-returning" blocks ("}") return @u (undefined). "Returning" blocks ("\!}") return the last value. |
| Comments | // single-line /\* multi-line \*/ | Human-readable descriptions of code. |
...
The \#i2 loop runs the main block (logging the current value of \#po2 to the console), runs a mid-iteration test that will stop iteration when the value has reached 1024, and then runs an "extra" block (doubling the value). Loops may also have pre and/or post tests, which run before the main block and after the "extra" block, respectively. (Use the post test instead of the mid test to test after the main block if you don't have an "extra" block.)

Note that different iterator names were used here for ease of reference; one iterator *could* have been used for both loops since they are not nested/concurrent in this example.
...
* %x, \#x, and \!x generate an error at runtime if "x" isn't set;  
  %?x, \#?x, and \!?x use else=@u internally to return @u (undefined) if "x" isn't set.
...
* Note that "\!}" is a single, indivisible, "lexical token", not two. It ends a code block, and is meant as a visual alert/reminder that the final value will be returned. It is unrelated to the \! (message parameter) namespace.  
  Further note that "\! }" (as two separate tokens) is an error, as \! is not permitted to appear by itself as a value (only as the base object for a message).

* Namespaces help avoid naming collisions, but you do have to be mindful when reading or writing code so as to not mix them up (or potentially even use an @ where you meant %, \#, or \!, or vice-versa).
...
* The outermost list container (the "SLID container") is wrapped in "\[(" and ")\]" instead of the usual plain "\[" and "\]". This allows it to be distinguished from executable Mesgjs code, as "\[(" is not a legal token sequence in Mesgjs (a message/chain must always begin with a base object).
